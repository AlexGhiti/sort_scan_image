...
class Paper:
	...
	def init_classifier(self):
		# 1/ Get the list of papers.
		list_papers = self.__get_paper_list()
	
		# 2/ Add them to corpus.
		for p in list_papers:
			paper_name = p.split("/")[-1]
			# a/ Check if paper exists in database.
			# TODO Get TFC from database
			if exists in database:
				self.paper_sort.add_preprocessed_paper_to_corpus(paper_name, p + ".txt", TFC)
			else:
				self.paper_sort.add_raw_paper_to_corpus(paper_name, p + ".txt")

		# 3/ Hand them to classifier !
		self.paper_sort.learn(debug = False)
